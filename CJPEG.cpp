...
#define EXIF_LEN (4 + 2 + 3 + 1 + 4  + 2 + (8*12) + 20 + 20 + 128 + 256 + 64 + 4)
...
/*  marker->emitword(CMarkerWriter::TAG_EXIF_VERSION);		// version... 2.20
  ...
  marker->emitword(0x0000);
	...
	marker->emitTextExif(m_autore,64-1);
...
BYTE *CJpeg::buildJPEG(const CBitmap *bmp, DWORD *len, BOOL reverseV, int quality, const char *note, bool ratio169, const char *autore) {
	register int i;
  BYTE *d=NULL,*s;
	DWORD l;
	BITMAPINFO *bi;
...
BYTE *CJpeg::buildJPEG(BITMAPINFO *bi, BYTE *pBuffer, DWORD *len, BOOL reverseV, int quality, const char *note, bool ratio169, const char *autore) {
	CBitmap b;
...
BYTE *CJpeg::readJPEGFile(const char *filename,BITMAPINFOHEADER *bmp, BYTE *theBytes,
													BYTE colorBits,bool ditherMode,bool reverseV) {
	CFile mF;
	BYTE *p;
